...
export async function deployToken(params: {
  ...
  try {
    ...
    const {
      o1js: { PrivateKey, PublicKey, UInt64, Mina, AccountUpdate, UInt8, Bool },
      zkcloudworker: {
        FungibleToken,
        FungibleTokenAdmin,
    ...
    const sender = PublicKey.fromBase58(adminPublicKey);
    ...
    const nonce = await getAccountNonce(adminPublicKey);
    ...
    await tx.prove();
    tx.sign([contractPrivateKey, adminContractPrivateKey]);
    ...
    if (signedData === undefined) {
      ...
      updateLogItem("transaction", {
        ...
        description: "No user signature received, deploy transaction cancelled",
        date: new Date(),
      });
      return {
        success: false,
