...
export interface TinyTransactionParams {
  chain: string;
...
export async function sendTinyTransaction(
  params: TinyTransactionParams
): Promise<string | undefined> {
  console.log(`Sending tiny transaction...`);
  const api = getAPI();
  ...
  const answer = await api.execute({
    ...
    transactions: [transaction],
    task: "tiny",
    args: JSON.stringify({ sender: params.sender }),
    metadata: `tiny transaction`,
  });
  console.log("answer:", answer);
  // TODO: handle errors and structure FungibleTokenJobResult
  const jobId = answer.jobId;
